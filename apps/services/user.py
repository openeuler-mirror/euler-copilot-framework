...
class UserManager:
    ...
    async def _handle_admin_user_creation(user_sub: str, user_name: str) -> str:
        ...
        try:
            ...
            if existing_admin:
                ...
                logger.info(
                    f"[_handle_admin_user_creation] 管理员用户已存在，使用原始user_sub: {user_sub}")
                return user_sub
            else:
                # 数据库中不存在管理员用户，使用配置的管理员user_sub
                logger.info(
                    f"[_handle_admin_user_creation] 管理员用户不存在，使用配置的user_sub: {config.admin.user_sub}")
    ...
    async def update_user_preferences_by_user_sub(user_sub: str, data: UserPreferencesRequest) -> None:
        ...
        if data.reasoning_model_preference is not None:
            preferences_update["preferences.reasoningModelPreference"] = data.reasoning_model_preference.model_dump(
                by_alias=True)
        if data.embedding_model_preference is not None:
            preferences_update["preferences.embeddingModelPreference"] = data.embedding_model_preference.model_dump(
                by_alias=True)
        if data.reranker_preference is not None:
            preferences_update["preferences.rerankerPreference"] = data.reranker_preference.model_dump(
                by_alias=True)
        if data.function_call_model_preference is not None:
            preferences_update["preferences.functionCallModelPreference"] = data.function_call_model_preference.model_dump(
                by_alias=True)
    ...
    async def get_user_preferences_by_user_sub(user_sub: str) -> UserPreferences:
        ...
        if user_data and "preferences" in user_data:
            ...
            if "reasoning_model_preference" in preferences_data:
                preferences_data["reasoningModelPreference"] = preferences_data.pop(
                    "reasoning_model_preference")
                migration_needed = True
            if "embedding_model_preference" in preferences_data:
                preferences_data["embeddingModelPreference"] = preferences_data.pop(
                    "embedding_model_preference")
                migration_needed = True
            if "reranker_preference" in preferences_data:
                preferences_data["rerankerPreference"] = preferences_data.pop(
                    "reranker_preference")
                migration_needed = True
            if "function_call_model_preference" in preferences_data:
                preferences_data["functionCallModelPreference"] = preferences_data.pop(
                    "function_call_model_preference")
                migration_needed = True
            if "search_method_preference" in preferences_data:
                preferences_data["searchMethodPreference"] = preferences_data.pop(
                    "search_method_preference")
                migration_needed = True
            if "chain_of_thought_preference" in preferences_data:
                preferences_data["chainOfThoughtPreference"] = preferences_data.pop(
                    "chain_of_thought_preference")
                migration_needed = True
            if "auto_execute_preference" in preferences_data:
                preferences_data["autoExecutePreference"] = preferences_data.pop(
                    "auto_execute_preference")
                migration_needed = True
