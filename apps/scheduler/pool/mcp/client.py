...
class MCPClient:
    ...
    async def _main_loop(
        ...
        if isinstance(config, MCPServerSSEConfig):
            ...
            logger.info("[MCPClient] MCP %s：创建Stdio客户端，工作目录: %s",
                        mcp_id, cwd.as_posix())
        ...
        try:
            ...
            try:
                ...
                if "cancel scope" in str(e).lower() or "different task" in str(e).lower():
                    ...
                    self.status = MCPStatus.STOPPED
                    logger.warning(
                        "[MCPClient] MCP %s：关闭时遇到TaskGroup问题（已知问题，忽略）", mcp_id)
                else:
                    self.status = MCPStatus.ERROR
                    logger.warning(
                        "[MCPClient] MCP %s：关闭时发生运行时错误: %s", mcp_id, e)
            except Exception as e:
                self.status = MCPStatus.ERROR
            ...
            if self.error_sign.is_set():
                ...
                if hasattr(self, 'task') and self.task.done():
                    try:
                        ...
                        logger.error(
                            "[MCPClient] MCP %s：主任务异常: %s", mcp_id, task_exc)
                        raise task_exc
