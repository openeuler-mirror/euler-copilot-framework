...
async def push_init_message(
    ...
    await queue.push_output(
        ...
        data=InitContent(feature=feature, createdAt=created_at).model_dump(
...
async def push_rag_message(
    ...
    try:
        ...
        try:
            ...
            await queue.push_output(
                ...
                data=TextAddContent(text=f"❌ 错误: {error_message}").model_dump(
                    exclude_none=True, by_alias=True),
            )
    ...
    task.tokens.full_time = round(datetime.now(
        UTC).timestamp(), 2) - task.tokens.time
...
async def _push_rag_chunk(task: Task, queue: MessageQueue, content: str) -> tuple[Task, RAGEventData | None]:
    ...
    try:
        ...
        try:
            ...
            logger.error("[Scheduler] RAG事件数据解析失败: %s, 原始内容: %s",
                         parse_error, raw_content[:100])
            return task, None
        ...
        try:
            if content_obj.event_type == EventType.TEXT_ADD.value:
                await queue.push_output(
                    ...
                    data=TextAddContent(text=content_obj.content).model_dump(
                        exclude_none=True, by_alias=True),
                )
            elif content_obj.event_type == EventType.DOCUMENT_ADD.value:
                if isinstance(content_obj.content, dict):
                    await queue.push_output(
                        ...
                        data=DocumentAddContent(
                            ...
                            documentAuthor=content_obj.content.get(
                                "author", ""),
                            documentName=content_obj.content.get("name", ""),
                            documentAbstract=content_obj.content.get(
                                "abstract", ""),
                            documentType=content_obj.content.get(
                                "extension", ""),
                            documentSize=content_obj.content.get("size", 0),
                            createdAt=round(content_obj.content.get(
                                "created_at", datetime.now(tz=UTC).timestamp()), 3),
                        ).model_dump(exclude_none=True, by_alias=True),
                    )
                else:
                    logger.warning(
                        "[Scheduler] DOCUMENT_ADD事件内容格式错误: %s", type(content_obj.content))
            else:
                logger.warning("[Scheduler] 未知的事件类型: %s",
                               content_obj.event_type)
        ...
        logger.error("[Scheduler] 处理RAG消息块时发生未知错误: %s, 内容: %s",
                     e, content[:100] if content else "None")
        return task, None
