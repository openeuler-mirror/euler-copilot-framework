...
class DirectReply(CoreCall, input_model=DirectReplyInput, output_model=DirectReplyOutput):
    ...
    async def _parse_variable_references(self, attachment_dict: Dict[str, Dict[str, str]]) -> Dict[str, List[str]]:
        ...
        for var_name, var_info in attachment_dict.items():
            ...
            display_name = var_info.get(
                'displayName', f"conversation.{var_name}")
            ...
            variable = await pool_manager.get_variable_from_any_pool(
                ...
                conversation_id=getattr(
                    self._sys_vars.ids, 'conversation_id', None)
            )

            if variable is None:
                logger.warning(
                    f"[DirectReply] 变量不存在: {display_name} (scope: {scope_str}, name: {actual_var_name})")
                continue

            # 检查是否为文件类型变量
            if variable.var_type not in [VariableType.FILE, VariableType.ARRAY_FILE]:
                logger.warning(
                    f"[DirectReply] 变量不是文件类型: {display_name}, 类型: {variable.var_type}")
                continue
            ...
            if file_ids:
                ...
                logger.info(
                    f"[DirectReply] 提取到文件变量 {display_name}: {len(file_ids)} 个文件")
            else:
                ...
                if variable_file_map:
                    ...
                    for var_name, file_ids in variable_file_map.items():
                        if len(file_ids) == 1:
                            ...
                            try:
                                # 从MongoDB获取文件信息
                                doc_collection = MongoDB.get_collection(
                                    "document")
                                doc_info = await doc_collection.find_one({"_id": file_id})
                                if not doc_info:
                                    logger.warning(
                                        f"[DirectReply] 文件信息不存在: {file_id}")
                                    continue

                                filename = doc_info.get(
                                    "name", f"file_{file_id}")
                                file_size = doc_info.get("size", 0)
                                file_type = doc_info.get(
                                    "type", "application/octet-stream")

                                # 从MinIO下载文件数据
                                try:
                                    metadata, file_data = MinioClient.download_file(
                                        "document", file_id)
                                    ...
                                    if "file_name" in metadata:
                                        try:
                                            original_filename = base64.b64decode(
                                                metadata["file_name"]).decode('utf-8')
                                    ...
                                    logger.info(
                                        f"[DirectReply] 成功返回单文件: {var_name}/{filename} (ID: {file_id})")

                                except Exception as minio_error:
                                    logger.error(
                                        f"[DirectReply] 从MinIO下载文件失败: {file_id}, 错误: {minio_error}")
                                    continue

                            except Exception as file_error:
                                logger.error(
                                    f"[DirectReply] 处理文件失败: {file_id}, 错误: {file_error}")
                                continue
                            ...
                            if files_data:
                                ...
                                logger.info(
                                    f"[DirectReply] 成功返回多文件: {var_name} ({len(files_data)} 个文件)")
                else:
