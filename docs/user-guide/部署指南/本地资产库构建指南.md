...
将文件存放的源目录映射至 RAG 容器目标目录，源目录的配置在  https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/values.yaml 中，下面是文件中具体配置映射源目录的的配置方法：

![配置映射源目录](./pictures/本地资产库构建/配置映射源目录.png)

中间层的配置（链接源目录和目标目录的配置）在 https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/templates/rag/rag-deployment.yaml 中，下面是文件中具体映射中间层的的配置方法：

![配置映射中间层](./pictures/本地资产库构建/配置映射中间层.png)

目标目录的配置在 https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/templates/rag/rag-deployment.yaml 中，下面是文件中具体映射目标目录的的配置方法：
...
**{dabase_url}**为 k8s 集群内访问 postgres 服务的 url，请根据具体情况修改，一般为 **{postgres_servive_name}-{{ .Release.Name }}.\<namespace\>.svc.cluster.local** 格式，其中 **{postgres_servive_name}** 可以从 https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/templates/pgsql/pgsql-service.yaml 找到：
...
**database_port** 的信息可以在 https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/templates/pgsql/pgsql-service.yaml 中查看，以下为字段所在位置（一般为5432）：
...
**{rag_url}** 为 0.0.0.0，**{rag_port}** 可以从 https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/templates/rag/rag-service.yaml 中获取（一般为8005）：
