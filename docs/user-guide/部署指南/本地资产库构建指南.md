...
将文件存放的源目录映射至 RAG 容器目标目录，源目录的配置在  <https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/euler_copilot/values.yaml> 中，下面是文件中具体配置映射源目录的的配置方法：

![配置映射源目录](./pictures/本地资产库构建/配置映射源目录.png)

中间层的配置（链接源目录和目标目录的配置）在 <https://gitee.com/openeuler/euler-copilot-framework/blob/master/euler-copilot-helm/chart/euler_copilot/templates/rag/rag-deployment.yaml> 中，下面是文件中具体映射中间层的的配置方法：
