...
设备需联网并符合 EulerCopilot 的最低软硬件要求。确认服务器、硬件、驱动等准备就绪后，即可开始环境准备工作。为了顺利进行后续操作，请按照指引，先进入我
...
|3| 安装部署工具    | `bash install_tools.sh v1.30.2+k3s1 v3.15.3`  |  安装helm、k3s工具 |
|4| 大模型准备      | 提供第三方 OpenAI 接口或基于硬件本都部署大模型   |   本地部署大模型可参考附录部分  |

## 域名准备

为确保 EulerCopilot 的正确部署和使用，请准备以下四个服务的域名：`minio`、`authhub`、`eulercopilot` 和 `witchaind`。这些子域名需属于同一个主域名下，例如 `minio.test.com`、`authhub.test.com`、`eulercopilot.test.com` 和 `witchaind.test.com`。
