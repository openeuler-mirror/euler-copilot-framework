...
- 登录 AuthHub
  ...
  AuthHub 的域名以 <authhub.test.com> 为例，AuthHub 的登录界面如下图所示：
...
请在浏览器中输入 https://$(host_ip):8080 或 <https://域名>（其中 port 默认值为8080，若更改则需相应调整）访问 openEuler Copilot System 网页，并尝试进行问答体验。
...
### 1. 构建 openEuler 专业知识领域的智能问答
  ...
  3. 将 `vim euler-copilot-helm/chart/databases/templates/pgsql/pgsql-deployment.yaml` 的 volumes 相关字段注释
