...
XPU Turbo 原名 sysHAX， 功能定位为K+X异构融合推理加速，主要包含两部分功能：
...
**推理动态调度**：
  ...
  - 对于推理任务，prefill阶段属于计算密集型任务，decode阶段属于访存密集型任务。因此从计算资源的角度来说，prefill阶段适合在GPU/NPU等硬件上执行，而decode阶段可以放在CPU等硬件上执行。

  - 此外，针对MOE模型，XPU Turbo还支持AF分离功能。XPU Turbo针对MOE模型可以将FFN阶段卸载至CPU侧，将Attention阶段卸载至xPU侧。通过卸载部分任务到CPU侧，从来带来总性能上的提升。  

**CPU推理加速**：在CPU上通过NUMA亲和、并行优化、算子优化等方式，来加速CPU推理性能。

XPU-Turbo共包含两部分交付件：

![xpu-turbo-deploy](pictures/xpu-turbo-deploy.png "xpu-turbo-deploy")
...
XPU Turbo目前支持在Nvidia、Ascend上进行部署。下文以在Nvidia上进行部署举例，另外读者也可以参考：

- PD分离：[Nvidia部署](https://gitcode.com/openeuler/sysHAX/blob/dev/docs/sysHAX_online_deployment_guide_on_CPU+GPU.md)

- PD分离：[Ascend部署（310p）](https://gitcode.com/openeuler/sysHAX/blob/dev/docs/sysHAX_online_deployment_guide_on_CPU+NPU.md)

如下为XPU Turbo在Nvidia上部署的流程：

### 3.1 安装NVIDIA Container Toolkit（容器引擎插件）
...
AF分离目前支持在Ascend NPU（910B系列）和沐曦GPU（metax C500）上的部署。由于正处于beta阶段，功能暂未完善，请关注 [sysHAX-adapter](https://gitcode.com/openeuler/sysHAX-adapter) 。尝鲜版部署流程可参考：

- AF分离：[910B 部署流程](https://gitcode.com/openeuler/sysHAX-adapter/blob/dev/docs/sysHAX_AF_online_deployment_guide_on_CPU+NPU.md)
- AF分离：[metax C500部署流程](https://gitcode.com/openeuler/sysHAX-adapter/blob/dev/docs/sysHAX_AF_online_deployment_guide_on_CPU+MUXI.md)
