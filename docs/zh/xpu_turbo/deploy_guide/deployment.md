...
当前XPU-Turbo支持在的CPU类型为鲲鹏920系列，支持三种部署模式：[纯CPU部署](https://gitcode.com/openeuler/sysHAX/blob/dev/docs/sysHAX_online_deployment_guide_on_CPU.md)，[CPU+GPU（Nvidia）部署](https://gitcode.com/openeuler/sysHAX/blob/dev/docs/sysHAX_online_deployment_guide_on_CPU+GPU.md)，[CPU+NPU（Ascend）部署](https://gitcode.com/openeuler/sysHAX/blob/dev/docs/sysHAX_online_deployment_guide_on_CPU+NPU.md)。下面将以CPU+GPU部署为例，其他部署方式可参考链接。
