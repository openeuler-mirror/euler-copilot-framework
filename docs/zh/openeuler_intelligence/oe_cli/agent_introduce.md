...
本手册聚焦 智能助手 cli（Witty Assistant） 智能体能力体系展开全面介绍，智能体系列是 Witty 平台面向垂直业务场景打造的智能交互工具，依托专属技术架构与 MCP 服务能力底座，深度适配业务场景需求，实现轻量化、场景化的智能服务落地。
现阶段 Witty 平台已集成LLM chat、基础运维Agent、已知问题分析Agent三款智能体，其中 LLM chat聚焦智能对话问询能力，为用户提供自然语言交互支撑；基础运维Agent是面向openEuler基础运维领域的专用工具，其核心优势在于整合两大 MCP 服务能力，实现 openEuler 系统基础运维全场景覆盖与轻量化知识检索的有机结合；已知问题分析Agent将面向已知问题诊断场景开发。手册通过标准化的能力说明与实操案例，为企业运维人员提供 “即查即用” 的操作指引，助力降低运维门槛、提升运维工作的标准化与高效化水平。
### 默认智能体总汇表

| Agent 名称    | 核心适用场景                            | 核心能力模块                                                          |
|-------------|-----------------------------------|-----------------------------------------------------------------|
| LLM chat    | 智能对话问询                            | LLM模型                                                           |
| 基础运维Agent   | 基础运维全场景（命令执行、文件管理、系统监控等）+ 轻量化知识检索 | 1. oe-cli-mcp-server：基础运维全能力工具集<br>2. rag-server：轻量化知识库管理与检索工具集 |
| 已知问题分析Agent | 针对已知问题的分析诊断                       | 待开发                                                             |

## 2. LLM chat

LLM chat 是 Witty 平台集成的智能对话类智能体，核心依托 LLM 大语言模型能力，无需依赖 MCP 服务支撑，专注为用户提供高效、自然的智能对话问询服务，是 Witty 平台智能交互体系的基础支撑模块。
...
基础运维Agent通过整合两大核心 MCP 服务，实现“基础运维 + 知识检索”的一站式运维中枢。所有工具均适配 openEuler 系统特性，支持本地/远程执行（部分工具）、中英文双语返回，具备严格的参数规范与统一的返回格式，可直接集成到企业运维流程中。
...
以下演示文件操作，提供自然语言交互 Prompt 格式，文件路径等关键信息即可使用，所有场景均贴合企业内部运维实际需求。
...
- 场景 ：本地文件批量管理
  ...
  ```text
    需要对本地 /root/test_python 目录下的文件进行管理，执行以下操作：
       ...
       2. 在 /root/test_python 目录下创建名为 backup_202601 的空文件夹；
       3. 将 /root/test_python 目录下所有 .py 后缀的文件移动到 backup_202601 文件夹中；
