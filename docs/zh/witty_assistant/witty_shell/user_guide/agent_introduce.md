...
|----------------|---------------|----------|--------------|--------------|
| witty_log_detection | **create_log_parse_task** | 日志解析任务创建器。支持创建不同类型的日志解析任务（基础/关键词检测/聚类检测/LLM检测），可指定日志文件范围、时间范围及异常检测规则。 | **必填：** `file_path_list`（日志文件路径列表）<br>**可选：** `task_type`（任务类型：base/log_detection_base_on_keywords/log_detection_base_on_clustering/log_detection_base_on_llm）、`query`（异常现象查询语句）、`max_anomaly_log_count`（最大异常日志数量，默认64）、`anomaly_keywords`（异常关键词列表）、`time_start`（时间起始，格式YYYY-MM-DD HH:MM）、`time_end`（时间结束，格式YYYY-MM-DD HH:MM） | `task_id`（uuid4格式的任务ID） |
| witty_log_detection | **get_task_message** | 任务信息查询器。查询指定日志解析任务的基本信息，包括任务状态、完成进度、创建时间及任务参数。 | **必填：** `task_id`（任务ID） | `task_id`（任务ID）、`task_name`（任务名称）、`task_type`（任务类型）、`completion_percent`（完成进度0.0-100.0）、`status`（任务状态）、`task_related_params`（任务参数JSON字符串）、`created_at`（创建时间） |
| witty_log_detection | **stop_task** | 任务终止器。终止指定ID的日志解析任务，返回操作是否成功。 | **必填：** `task_id`（任务ID） | `success`（是否成功，布尔值） |
| witty_log_detection | **get_task_result** | 任务结果获取器。分页查询指定日志解析任务的结果，可筛选异常/正常日志，返回日志内容、异常原因及异常分数等信息。 | **必填：** `task_id`（任务ID）<br>**可选：** `offset`（分页偏移量）、`limit`（分页返回数量）、`is_anomalous`（是否仅返回异常日志） | `total`（总结果数量）<br>**results（列表）：** `id`（日志解析结果ID）、`file_path`（日志文件路径）、`task_id`（任务ID）、`is_anomalous`（是否异常）、`content`（日志内容）、`anomaly_reason`（异常原因）、`anomaly_score`（异常分数0.0-100.0） |
