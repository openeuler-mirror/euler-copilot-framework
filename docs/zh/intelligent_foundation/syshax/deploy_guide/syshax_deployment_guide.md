...
|sysHAX|CPU|代理用户的请求，将Prefill请求发往vllm-gpu。根据负载均衡的情况，将Decode请求发往vllm-cpu或vllm-gpu|
...
|docker|25.0.3及以上|用docker将vllm-cpu和vllm-gpu的配置环境隔离，避免互相干扰。|
|模型|大部分主流模型|文档中以DeepSeek\-R1\-Distill\-Qwen\-32B为例介绍。|
...
|**类型**|**型号**|**说明**|
|---|---|---|
|GPU|Nvidia A100|vllm-gpu运行在A100上|
...
首先，把模型存放在宿主机的/home/models路径下。然后，在创建容器时，将/home/models挂载到容器内的相同路径，就可以在容器内访问模型了。推理前，需要先搭建vllm-gpu容器、vllm-cpu容器、sysHAX服务。
...
# 从远端拉取vllm-gpu的docker镜像
docker pull hub.oepkgs.net/neocopilot/syshax/syshax-vllm-gpu:0.2.1

# 创建名为vllm_gpu的docker容器，设置ipc为可共享，方便跟vllm-cpu共享ipc资源。创建结束后自动进入容器的工作目录（此命令中工作目录为/home）
...
|---|---|
|`name` |指定容器名。|
|`ipc` |ipc命名空间，用来使能vllm-cpu和vllm-gpu之间的通信。有两种ipc通信方式。为了方便，假设两个容器的name分别设置为vllm1和vllm2。第一种通信方式：vllm1、vllm2的ipc分别设置为"`shareable`"、"`container:vllm1`"，表示vllm1和vllm2共享ipc命名空间；第二种通信方式：vllm1和vllm2的ipc都设置为"`host`"，表示两个容器跟宿主机共享ipc命名空间。|
...
|`hub.oepkgs.net/neocopilot/syshax/syshax-vllm-gpu:0.2.1`|指定使用的容器镜像名称及版本。`hub.oepkgs.net/neocopilot/syshax/syshax-vllm-gpu:0.2.1`镜像已经配置好了vllm-gpu所需的环境，使用此镜像无需再进行配置|
...
|`enable_auto_pd_offload`|支持sysHAX的PD分离功能。|
...
#部署vllm-cpu服务。要开启pd分离，要添加--enable_auto_pd_offload
...
|**参数**|**解释**|
|---|---|
|`INFERENCE_OP_MODE`|自定义环境变量，表示是否启动CPU推理加速。可选值：`fused`、`None`。|
|`OMP_NUM_THREADS`|自定义环境变量，表示CPU推理加速时开启的线程数量。其值小于等于可用的CPU核数量。用lscpu命令可查看服务器中的CPU核数。|
|`CUSTOM_CPU_AFFINITY`|自定义环境变量，表示将线程的绑核方式。注意此处的线程数量要跟OMP\_NUM\_THREADS一致，并且每个numa的线程数量要相等。格式为“start\-end:step”，step默认为1。例如，0\-159:2中，0、159、2分别代表开始的线程序号、结束的线程序号（包含）、步长。|
|`SYSHAX_QUANTIZE`|自定义环境变量，表示采用的量化方式。开启量化时，可选值为`q8_0`、`q4_0`，表示进行q8\_0量化、q4\_0量化。其他设置表示不量化。|
|`NRC`|自定义环境变量，表示i8mm指令处理的矩阵分块的大小。可选值为`2`、`4`。|
|`/home/models/DeepSeek-R1\-Distill-Qwen-32B`|模型存储路径。|
|`host` |指定服务器监听的网络接口。|
|`port` |指定服务器监听的网络端口号。|
|`dtype`|指定模型权重加载的数据类型。可选值为`half`。|
...
在sysHAX场景中，用户需要将请求发往sysHAX服务。sysHAX根据CPU和GPU的特性（CPU内存大但是算力弱，GPU内存小但是算力强）分配Prefill和Decode请求。sysHAX采用了PD分离的技术，因此需要vllm-cpu和vllm-gpu也各自支持PD分离。启动vllm-cpu和vllm-gpu的服务时加上启动参数enable\_auto\_pd\_offload，就可以让它们支持PD分离特性。

目前已经搭建好vllm-cpu容器和vllm-gpu容器了，现在搭建sysHAX服务对vllm-cpu、vllm-gpu实现异构调度。
...
# 配置sysHAX的端口。sysHAX代理用户请求，即用户将请求发给sysHAX，sysHAX再决定将请求发给vllm-cpu还是vllm-gpu
python3 cli.py config conductor.port 8010

# 配置sysHAX是否开启自动 PD offload（true/false）。如果需要开启PD分离，在GPU上做Prefill，在CPU和GPU上做Decode，则开启；否则关闭
