...
```
## 环境准备
设备需联网并符合EulerCopilot的最低软硬件要求。确认服务器、硬件、驱动等准备就绪后，即可开始环境准备工作。为了顺利进行后续操作，请按照指引，先进入脚本部署目录`/home/euler-copilot-framework/euler-copilot-helm/scripts`，并且按照提供的操作步骤和脚本路径依次执行，以确保初始化成功。
