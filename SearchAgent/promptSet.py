prompt={}
prompt['splitQ'] = """
现在是2024年，你是一个拆分问题的智能助手，负责将"需拆分问题"拆分为可通过搜索回答的子问题。

## 任务介绍
将问题拆分为多个单一、具体的子问题，便于搜索获取答案。每个子问题应涉及一个特定的知识点、时间、地点、人物或事件。请避免复合问题（如多个知识点的问题或者多个事物的比较加筛选）或重复提问（提问已经提问过的问题）。子问题必须完整独立，避免使用模糊表达，如"该组织，以上内容"。


## 返回格式
返回一个列表，每个元素为一个子问题。如果不可拆分，请返回空列表。

## 样例
样例问题：什么是生成式 AI？
答案：["生成式模型定义", "生成式模型的运作原理是什么？", "生成式模型的应用有哪些？"]
样例问题：中国的2023年的GDP是多少？
答案：[]
...
prompt['summary'] = """
根据问题和拆解出的子问题，筛选有助于回答问题的网页内容，并在300字内总结。每个关键点需标明网页ID，格式为`[[int]]`，多个ID用`[[id_1]][[id_2]]`表示。

- 回答需逻辑清晰、结构分明，易于理解。
- 确保每个关键点引用对应的搜索结果来源，保持专业、严谨的表达。
- 回答必须完整，避免使用模糊表达，如"基于上述内容"，最终展示的内容不包含网页内容。
- 统一语法和词汇，保持文档一致性和连贯性。
- 由于网页过长，网页内容可能被截断，请专注于抽取关键信息。

用户问题：{question} {answers} {subq}
"""


prompt['subq'] = "\n子问题：{subq} {answers} {subqLast}"

prompt['pageItem'] = "\n搜索到的网页ID：{id} 网页内容：{content}"
